...
Questions:
    ...
    1. What is worst-case time complexity of Map.insert()?
        ...
        O(n) but O(1) amortized and average complexity.
    ...
    3. What is the load factor of a hash table?
        ...
        The load factor of a hash table is the number of key-value
        pairs inside the table (size) divided by the number of buckets
        the hash table currently has.
    ...
    do internally when the load factor reaches a certain threshold?
        ...
        When the load factor reaches a certain threshold, the hash table
        must be resized to have more buckets.
'''
...
class Map:
    ...
    def insert(self, key, value):
        ...
        bucket = self.buckets[Map.hash(key) % len(self.buckets)]
        for pair in bucket:
            if key == pair.key:
                pair.value = value
                break
    ...
    def resize(self):
        ...
        True
        '''
        new = [[] for _ in range(2 * len(self.buckets))]
        for bucket in self.buckets:
            for pair in bucket:
                new_bucket = new[Map.hash(pair.key) % len(new)]
                new_bucket.append(pair)
        self.buckets = new
