/* Title: dll-accounts-stretch.c
 * Abstract: This program extends dll-accounts.c with more operations.
 * Author: Eamon Tracey
...
#include "dll-accounts-stretch.h"
...
/*
 * Display the oddly numbered accounts in a doubly linked list.
 */
void displayOdd(dllist *list) {
	dlnode *curr = list->head;
	int n = 0;
...
void reverseList(dllist *list) {
	...
	// Traverse list, switching next and previous nodes.
	while (curr != NULL) {
		ref = curr->prev;
		curr->prev = curr->next;
		curr->next = ref;
...
/*
 * Move head node to the tail of a doubly linked list.
 */
void moveFirstToLast(dllist *list) {
	if (list->head == NULL || list->head == list->tail) {
		return;
	}
